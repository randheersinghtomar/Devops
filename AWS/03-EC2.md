# 🚀 Amazon EC2 (Elastic Compute Cloud)

> **Amazon EC2 Documentation | Interview Preparation | Production Notes**


# 📖 Introduction

Amazon Elastic Compute Cloud (Amazon EC2) is one of the core AWS services that provides virtual servers in the cloud. It enables users to deploy applications without purchasing or maintaining physical hardware.

Amazon EC2 is designed to provide scalable, secure, and highly available compute resources for running applications of any size.

---

# 🚀 What is Amazon EC2?

### Definition

Amazon EC2 is a web service that provides secure and resizable virtual servers in the AWS Cloud.

### Explanation

Amazon EC2 allows you to launch Linux or Windows virtual machines within minutes. These virtual machines are called **Instances** and can be used to host websites, APIs, databases, enterprise applications, containerized workloads, and development environments.

Instead of purchasing expensive physical servers, organizations rent compute resources from AWS and pay only for the resources they consume.

### Common Use Cases

- Web Hosting
- Application Servers
- Development & Testing
- CI/CD Pipelines
- Big Data Processing
- Machine Learning Workloads
- Backup Servers

### Real World Example

A company launches multiple EC2 instances behind an Application Load Balancer to host its production website. During high traffic events, additional EC2 instances are automatically launched using Auto Scaling.

---

# 💻 Compute

### Definition

Compute refers to the processing resources required to run an application.

### Explanation

Compute represents the CPU, memory (RAM), operating system, and processing power required for executing workloads.

Without compute resources, applications cannot perform calculations, process requests, or serve users.

### Compute Resources Include

- CPU
- RAM
- Operating System
- Network Performance
- Storage Connectivity

### Real World Example

A Java application requires CPU for processing requests and RAM for storing active sessions. Amazon EC2 provides these compute resources without requiring physical hardware.

---

# ⭐ Amazon EC2 Features

Amazon EC2 offers the following capabilities:

- Resizable Compute Capacity
- Multiple Instance Types
- Multiple Operating Systems
- High Availability
- Elastic Scaling
- Secure Access
- Pay-As-You-Go Pricing
- Global Infrastructure
- Integration with IAM
- Integration with CloudWatch
- Integration with Elastic Load Balancer
- Integration with Auto Scaling

### Production Example

An enterprise application running on EC2 can automatically scale during business hours while remaining highly available across multiple Availability Zones.

---

# 📈 Scalability

### Definition

Scalability is the ability to increase or decrease computing resources based on business requirements.

### Explanation

As application traffic grows, additional resources are required to maintain performance. Scaling ensures that the application continues serving users efficiently.

There are two types of scaling.

## Vertical Scaling

Increasing the CPU or RAM of an existing EC2 instance.

Example

```
2 vCPU
4 GB RAM

↓

4 vCPU
8 GB RAM
```

Suitable for applications that require more processing power on a single server.

---

## Horizontal Scaling

Adding additional EC2 instances to distribute the workload.

Example

```
Before

        EC2

↓

After

EC2      EC2      EC2
```

Horizontal scaling improves both performance and availability.

### Production Example

During an online shopping festival, an e-commerce platform increases the number of EC2 instances from 5 to 50 to handle millions of customer requests.

---

# ⚡ Elasticity

### Definition

Elasticity is the automatic adjustment of compute resources based on workload demand. this can be done by configuring an EC2 Auto Scaling Group (ASG)

### Explanation

Elasticity automatically provisions additional resources during traffic spikes and removes unnecessary resources when demand decreases.

Unlike manual scaling, elasticity minimizes operational effort while optimizing cost.

Example

```
Morning

2 EC2

↓

Afternoon

15 EC2

↓

Night

2 EC2
```

### One important point
Auto Scaling Group ≠ Load Balancer

The Load Balancer distributes traffic, while the Auto Scaling Group adds/removes EC2 instances.

### Production Example

A food delivery application automatically launches additional EC2 instances during lunch and dinner hours and terminates them after peak traffic ends.

---

# 🌍 High Availability

### Definition

High Availability is the ability of an application to remain accessible even when a component fails.

### Explanation

High Availability is achieved by deploying EC2 instances across multiple Availability Zones. If one Availability Zone becomes unavailable, traffic is automatically redirected to healthy instances.

Example

```
Availability Zone A

EC2

        │
Application Load Balancer
        │

EC2

Availability Zone B
```

### Benefits

- Reduced downtime
- Improved reliability
- Fault tolerance
- Better customer experience

### Production Example

Internet banking applications deploy EC2 instances across multiple Availability Zones to ensure uninterrupted services even during infrastructure failures.

---

# 💰 Pay As You Go

### Definition

Pay only for the resources that you actually consume.

### Explanation

Amazon EC2 follows a usage-based pricing model where customers are billed only for the compute resources used.

There is no upfront investment for purchasing hardware.

### Benefits

- No Capital Expenditure
- Cost Optimization
- Flexible Scaling
- Suitable for Startups and Enterprises

### Production Example

A startup runs development servers only during office hours and stops them after work, significantly reducing cloud costs.

---

# 🧩 Amazon EC2 Components

An EC2 instance is created using multiple AWS components.

| Component | Purpose |
|------------|----------|
| AMI | Operating System Template |
| Instance Type | CPU & Memory Configuration |
| Key Pair | Secure Authentication |
| Security Group | Virtual Firewall |
| VPC | Networking |
| EBS Volume | Persistent Storage |
| IAM Role | Secure AWS Service Access |
| Tags | Resource Identification |

Understanding these components is essential for successfully launching and managing EC2 instances.

---

# 💿 Amazon Machine Image (AMI)

### Definition

An Amazon Machine Image (AMI) is a pre-configured template used to launch EC2 instances.

### Explanation

An AMI contains the operating system, application software, libraries, configuration files, and other settings required to launch an EC2 instance.

Instead of configuring every server manually, organizations create standardized AMIs for consistent deployments.

### Types of AMI

- AWS Managed AMI
- AWS Marketplace AMI
- Custom AMI

### Production Example

A company installs Ubuntu, Java, Docker, CloudWatch Agent, security patches, and monitoring tools on one server.

Instead of repeating the installation every time, they create a Custom AMI and launch all future servers using that image.

This ensures consistency across environments.

---

# EC2 Instance Types

## Definition

An EC2 Instance Type defines the hardware configuration and capacity of an EC2 instance., such as:

- CPU
- Memory (RAM)
- Storage performance
- Network capacity
- GPU/Accelerators

Different applications require different instance types.

---

# EC2 Instance Families

| Family | Type | Best For | Easy Real-Life Example |
|---|---|---|---|
| **T** | Burstable General Purpose | Low/variable CPU | Small website |
| **M** | General Purpose | Balanced workloads | Company application |
| **C** | Compute Optimized | High CPU | Video processing |
| **R** | Memory Optimized | High RAM | Large database |
| **X** | High Memory | Very high RAM | Large in-memory database |
| **U** | High Memory | Extremely high RAM | Huge enterprise database |
| **I** | Storage Optimized | Fast storage/I/O | High-speed database |
| **D** | Dense Storage | Large storage | Big data/file storage |
| **G** | GPU | Graphics workloads | Gaming/3D graphics |
| **P** | Powerful GPU | Heavy AI/ML | AI model training |
| **F** | FPGA | Custom acceleration | Specialized financial/scientific processing |

Shortcut:-

**M/T** → General (Balanced)  
**C** → Compute (High CPU)  
**R/X/U** → Memory (High RAM)  
**I/D/H** → Storage  
**P/G/F/Inf/Trn** → Accelerated
---

## 1. T – Burstable General Purpose

**Best for:** Applications that usually need low CPU but sometimes need extra CPU power.

### Use Cases

- Small website
- Development and testing
- Personal blog
- Low-traffic application

**Example:** `t3.micro`

### Real-Life Example

A **small company website** usually has few visitors, so it needs low CPU. During a sale or promotion, traffic increases and it temporarily needs more CPU.

> **Remember: T = Temporary CPU Burst**

---

## 2. M – General Purpose

**Best for:** Applications needing a balanced amount of CPU and RAM.

### Use Cases

- Web server
- Application server
- Business application
- Small/medium company applications

**Example:** `m6i.large`

### Real-Life Example

A company runs an **employee management application** where users log in, check attendance, apply for leave, and view reports. It needs a balanced amount of CPU and RAM.

> **Remember: M = Most Balanced**

---

## 3. C – Compute Optimized

**Best for:** Applications requiring high CPU processing power.

### Use Cases

- Video encoding
- Batch processing
- Gaming servers
- Scientific calculations

**Example:** `c6i.large`

### Real-Life Example

A company uploads thousands of videos and needs to **convert them into different formats**. This requires heavy CPU processing, so a C instance is suitable.

> **Remember: C = Compute / CPU**

---

## 4. R – Memory Optimized

**Best for:** Applications requiring a large amount of RAM.

### Use Cases

- Large databases
- Caching
- In-memory applications
- Data analytics

**Example:** `r6g.large`

### Real-Life Example

An **e-commerce website** has millions of products and users. Frequently used data can be kept in memory for faster access. This requires high RAM.

> **Remember: R = RAM**

---

## 5. X – High Memory

**Best for:** Applications requiring **very large amounts of RAM**.

### Use Cases

- Large in-memory databases
- SAP HANA
- Large enterprise applications

### Real-Life Example

A large company needs to keep a **huge amount of business data in memory** so that reports and transactions can be processed very quickly.

> **Remember: X = eXtra Memory**

---

## 6. U – High Memory

**Best for:** Applications requiring **extremely large amounts of RAM**.

### Use Cases

- Very large SAP HANA workloads
- Huge enterprise databases

### Real-Life Example

A multinational company runs an extremely large database containing data from **millions of customers and transactions** and needs a huge amount of memory.

> **Remember: U = Ultra-High Memory**

---

## 7. I – Storage Optimized

**Best for:** Applications requiring **very fast local storage and high I/O performance**.

### Use Cases

- High-performance databases
- Data processing
- Applications with frequent disk access

### Real-Life Example

A database application receives and reads **thousands of transactions every second**. It needs very fast disk access.

> **Remember: I = I/O Intensive**

---

## 8. D – Dense Storage

**Best for:** Applications requiring a **large amount of local storage**.

### Use Cases

- Big data
- Data warehouses
- Large file storage

### Real-Life Example

A company stores **years of logs, images, videos, and other large files** and needs a large storage capacity.

> **Remember: D = Dense Storage**

---

## 9. G – GPU / Graphics

**Best for:** Graphics and GPU-based workloads.

### Use Cases

- Gaming
- 3D graphics
- Video rendering
- Machine learning inference

**Example:** `g5.xlarge`

### Real-Life Example

A **cloud gaming platform** needs powerful GPUs to process game graphics and deliver them to users.

> **Remember: G = Graphics**

---

## 10. P – Powerful GPU

**Best for:** Heavy AI and Machine Learning workloads.

### Use Cases

- AI model training
- Deep learning
- Image recognition
- Large-scale ML

### Real-Life Example

A company is training an **AI model to recognize faces or detect objects in millions of images**. This requires powerful GPUs.

> **Remember: P = Powerful GPU**

---

## 11. F – FPGA

**Best for:** Specialized workloads requiring custom hardware acceleration.

### Use Cases

- Scientific computing
- Financial calculations
- Specialized data processing

### Real-Life Example

A financial company needs to process **millions of market calculations extremely quickly** and uses custom hardware acceleration.

> **Remember: F = FPGA**

---

# Production Examples

- `t3.micro` → Learning, development, small websites, and variable workloads
- `m6i.large` → Business applications and balanced workloads
- `c6i.large` → Compute-intensive applications and video processing
- `r6g.large` → Memory-intensive applications and databases
- `g5.xlarge` → Gaming, graphics, and GPU-based machine learning workloads

---

# Final Interview-Friendly Summary

```text
EC2 Instance Type
        │
        ├── T → Burstable General Purpose
        │         Example: Small website with occasional high traffic
        │
        ├── M → Balanced General Purpose
        │         Example: Business application
        │
        ├── C → Compute Optimized
        │         Example: Video processing
        │
        ├── R → Memory Optimized
        │         Example: Large database
        │
        ├── X → Very High Memory
        │         Example: Large in-memory database
        │
        ├── U → Extremely High Memory
        │         Example: Huge enterprise database
        │
        ├── I → High-Speed Storage / I/O
        │         Example: High-performance database
        │
        ├── D → High-Capacity Dense Storage
        │         Example: Big data and large file storage
        │
        ├── G → GPU / Graphics
        │         Example: Gaming and 3D graphics
        │
        ├── P → Powerful GPU for AI/ML
        │         Example: AI model training
        │
        └── F → FPGA Acceleration
                  Example: Specialized financial processing
```
---                 
# 🔑 Key Pair

### Definition

A Key Pair is a set of cryptographic keys used to securely access an EC2 instance.

### Explanation

A Key Pair consists of:

- Public Key
- Private Key (.pem)

The public key is stored on the EC2 instance, while the private key is downloaded by the user during key pair creation.

When connecting to a Linux EC2 instance using SSH, AWS verifies that the private key matches the stored public key.

### Authentication Flow

```
Private Key (.pem)
        │
        ▼
SSH Authentication
        │
        ▼
Public Key on EC2
        │
        ▼
Access Granted
```

### Linux Login

```bash
ssh -i my-key.pem ec2-user@Public-IP
```

### Windows Login

Windows instances generally use **RDP (Remote Desktop Protocol)** with an administrator password decrypted using the key pair.

### Best Practices

- Store the private key securely.
- Never share the `.pem` file.
- Restrict file permissions (`chmod 400 key.pem`).
- Use Systems Manager Session Manager whenever possible instead of direct SSH.

### Production Example

An administrator connects to a production Linux server using SSH with a private key instead of using passwords.

---

# 🛡 Security Group

# Security Groups

It acts as a virtual firewall for your EC2 instances (& some other services) to control inbound & outbound traffic.

- Inbound Controls what traffic is allowed into your instance.  
- Oiutbound Controls what traffic your instance can send out.  
- By default all outbound traffic is allowed.

## Example: Web Server
- **Inbound:** Allows HTTP (80) & HTTPS (443) from anywhere.  
- **Outbound:** Default (allow all).

**(SG का काम है फिल्टर का traffic पहले ही रोकना जिससे बेकार का process ना भेजना पड़े server को। जैसे HTTP HTTPS के अलावा कोई भी traffic को रोक देना (eg: smtp) पहले ही रोकना (चाहे client वो भी already web server के लिए नहीं है।))**

This filtering is done to reduce extra load & secure the server.

--

# Two ways to make SG
1. We can create SG while making EC2.  
2. We can create SG separately & then add it to instance.  

*Both have different inbound and outbound default rules

## Default SG instance

- 1st method default rule→ Outbound (All traffic allowed),
                           Inbound {for windows-(RDP 3380) & for Linux (ssh 22)  

- 2nd method default rule→  Outbound (All traffic allowed),
                           Inbound ( No rule/ Nothing allowed) 


**Note:** We cannot add Deny rule.We can only add allow rule.  
- e.g. if hhtp is allowed then all other rules are automatically denied.  
--

# Stateful / Stateless
- SG is stateful (inbound & outbound processes)
- In stateful, once connection is established, the return traffic is automatically allowed. 
- NACL is stateless (inbound & outbound processes)  
- In stateless, the return traffic is not allowed, you must manually allow inbound & outbound traffic.  

--

## Example (SG)
- You create an inbound rule to allow HTTP traffic.  
- When request comes to instance, the EC2 responds automatically.  
- The EC2 can send an outbound response.  
- This is stateful because connection allows the return traffic.  

## Example (NACL)
- You create an inbound rule to allow HTTP traffic.  
- When request comes to instance, the EC2 responds automatically.  
- The EC2 cannot send an outbound response unless outbound rule is also set.  
- This is stateless because connection does not allow return traffic.  

--
# Network ACL vs Security Groups

| Feature | Security Groups (SG) | Network ACL (NACL) |
|---------|----------------------|--------------------|
| Stateful?| Yes | No (stateless) |
| Return traffic allowed? | Automatic | Must add explicit inbound and outbound rules. |
| Rule direction needed | Only need to set the direction you initiate | Both directions) |

--

### Common Rules

| Protocol | Port | Purpose |
|----------|------|----------|
| SSH | 22 | Linux Remote Login |
| RDP | 3389 | Windows Remote Login |
| HTTP | 80 | Website |
| HTTPS | 443 | Secure Website |

### Example

```
Internet
     │
     ▼

Security Group

22  ✔ Office IP

80  ✔ Everyone

443 ✔ Everyone

3306 ❌ Blocked
```

### Best Practices

- Allow SSH only from trusted IP addresses.
- Never allow unnecessary ports.
- Follow the Principle of Least Privilege.
- Separate Security Groups for Web, Application and Database servers.

### Production Example

A production web server allows:

- HTTP (80) from Everyone
- HTTPS (443) from Everyone
- SSH (22) only from the company's VPN IP.

---

# 🌐 Elastic IP

### Definition

An Elastic IP is a static public IPv4 address provided by AWS.

### Explanation

Normally, a Public IP changes when an EC2 instance is stopped and started.

Elastic IP ensures the public IP remains the same.

This is useful for applications where clients, DNS records, or firewalls depend on a fixed IP address.

### Example

Without Elastic IP

```
Today

54.x.x.x

↓

Restart

13.x.x.x
```

With Elastic IP

```
54.x.x.x

↓

Restart

54.x.x.x
```

### Best Practices

- Allocate Elastic IP only when required.
- Release unused Elastic IPs to avoid unnecessary charges.
- Associate Elastic IP with production servers requiring static public access.

### Production Example

A banking application maps its DNS record to an Elastic IP so that customers always connect to the same public IP.

---

# 🔄 EC2 Lifecycle

### Definition

The EC2 Lifecycle represents the different operational states of an EC2 instance.

### Lifecycle

```
Launch
   │
Pending
   │
Running
├── Reboot
├── Stop
├── Start
├── Hibernate
└── Terminate
```

### State Description

| State | Description |
|--------|-------------|
| Pending | Instance is launching |
| Running | Instance is operational |
| Reboot | Operating System restarts |
| Stop | Instance stops but EBS remains |
| Start | Starts a stopped instance |
| Hibernate | Saves RAM to EBS before stopping |
| Terminate | Permanently deletes the instance |

### Important Notes

- Reboot does not change the Instance ID.
- Stop retains EBS volumes.
- Terminate deletes the instance (unless Delete on Termination is disabled for attached volumes).
- Hibernate resumes applications from the previous memory state.

### Production Example

Development servers are stopped after office hours to reduce compute costs while retaining their EBS volumes.

---

# 📜 User Data script

( You can find this option in advanced details option while creating the EC2 instance.)

### Definition

User Data is a bootstrap script that runs automatically when an EC2 instance launches.

### Explanation

User Data automates software installation and initial server configuration.

This removes manual configuration after launching an instance.

### Example Script

```bash
#!/bin/bash

yum update -y

yum install httpd -y

systemctl enable httpd

systemctl start httpd

echo "Welcome to AWS EC2" > /var/www/html/index.html
```

### Best Practices

- Keep User Data scripts idempotent where possible.
- Use version-controlled scripts.
- Avoid storing passwords or secrets directly in User Data.

### Production Example

Every new web server automatically installs Apache, configures services, and deploys the application during launch.

---

# 💰 Purchase Options

### Definition

Purchase Options determine how you use/pay for EC2 instances.

### Available Options

# EC2 Instance Purchase Options in detail

| Feature | On-Demand | Spot | Reserved (Standard) | Reserved (Convertible) | Reserved (Scheduled) | Savings Plans (EC2) | Savings Plans (Compute) |
|----------|------------|------|---------------------|------------------------|----------------------|---------------------|--------------------------|
| **Pricing Model** | Pay-as-you-go | Bidding model | No upfront / Partial upfront / All upfront | No upfront / Partial upfront / All upfront | No upfront / Partial upfront / All upfront | No upfront / Partial upfront / All upfront | No upfront / Partial upfront / All upfront |
| **Commitment Duration** | None | None | 1 or 3 years | 1 or 3 years | 1 year | 1 or 3 years | 1 or 3 years |
| **Cost Savings** | None | 90% | 72% (1 yr, all upfront) | 72% (1 yr, all upfront) | 70% | 72% | 66% (All up) |
| **Instance Interruption** | — | Yes | — | — | — | — | — |
| **Change Size (t2.small to t2.medium)** | — | — | Yes | Yes | — | Yes | Yes |
| **Change Family (t2 to c5)** | — | — | No | Yes | No | No | Yes |
| **Change Compute (EC2 to Lambda)** | — | — | No | No | No | No | Yes |
| **Change OS** | — | — | No | Yes | No | No | Yes |
| **Change Region** | — | — | No | No | No | No | Yes |
| **Change Tenancy** | — | — | No | No | No | No | Yes |



| Option | How it works | When to use |
|---------|---------------|-------------|
| On-Demand | Pay by the second/hour for exactly what you run. No upfront cost or long-term commitment. | Short-term or unpredictable workloads. Good for starting out or dev/test. |
| Spot | Use AWS’s unused capacity at up to 90% discount. AWS can reclaim the instance with 2-minute notice. | Flexible, fault-tolerant tasks like batch jobs, big data analysis, CI/CD. |
| Reserved (Standard) | Commit to a specific instance type and region 1/3 years. Up to 72% cheaper than On-Demand. | Steady predictable workloads. |
| Reserved (Convertible) | Similar to standard but you can change instance family or tenancy during the term. | Long-running workloads that may evolve. |
| Reserved (Scheduled) | Reserve capacity that repeats on a schedule (e.g., every Monday 9–5). | Workloads with regular recurring time windows. |
| Savings Plans – EC2 | Commit to a specific instance family in a region for 1 or 3 years; flexible across sizes/os. | Consistent usage within one family but want more flexibility than reserved instances. |
| Savings Plans – Compute | Commit to a $ amount of compute per hour, not a specific family. Applies to EC2, Fargate, Lambda. | Max flexibility across instance types, region, and even services. |

Note:- AWS Pricing Calculation is used to know the price

# EC2 Tenancy / Hardware Isolation

It refers to how your EC2 instance are provisioned on the physical hardware within an amazon data center. This choice is critical as it affects factors like cost, security, compliance and performance.

(can be selected in EC2 → Advanced details → Tenancy)  

There are 3 main types of tenancy in AWS –

| Tenancy Type | Description | Pros | Cons |
|---------------|-------------|------|------|
| **Shared Tenancy (default)** (lowest price) | Standard, most common, and most cost-effective option. Instances run on physical h/w that may also be hosting instances belonging to other AWS customers. | Lowest cost, highly flexible, and simple to use. This is default setting. | While AWS uses strong security measures to isolate instances, some strict regulatory or compliance requirements may not permit. |
| **Dedicated Instances** | EC2 instance that runs on h/w exclusively dedicated to a single AWS account. Instances from your own account can still run on the same h/w. AWS chooses the host. Pay a small hourly “dedicated instance fee” per region + normal instance costs. | Provides a higher level of isolation & security, suitable for apps with specific compliance or regulatory requirments (E.g., HIPAA). Offers a balance b/w cost and isolation. | More expensive than shared tenancy & may not provide the same level of granular control as dedicated host. |
| **Dedicated Hosts** | A whole physical server allocated to you. Gives most control & visibility over the server. You can bring your own eligible software licenses (BYOL). Pay for the entire host, not per instance. | Max isolation & control over instance placement. You have visibility into the number of sockets & physical cores. Crucial for managing regulatory requirements per-socket, per-case, per-VM software licenses. | Most expensive. Pay for entire physical server. |



### Best Practices

- Use On-Demand for development.
- Use Reserved Instances or Savings Plans for long-running production workloads.
- Use Spot Instances for batch processing and CI/CD jobs.

### Production Example

A media company uses Spot Instances to process video files because interruptions can be tolerated.

---

# 📍 Placement Groups

### Definition

Placement Groups define how EC2 instances are physically placed within AWS infrastructure.

### Types of Placement Groups

```text
                 Placement Groups
                        |
          +-------------+-------------+
          |             |             |
          v             v             v
       Cluster        Spread      Partition
          |             |             |
     High network     Maximum       Groups of
      performance     isolation      instances
```

## Cluster Placement Group

Purpose: Place instances close together on the underlying infrastructure.
```text
+----------------------------------+
|        Same physical area       |
|                                  |
|  EC2 ---- EC2 ---- EC2 ---- EC2 |
|          Fast network            |
+----------------------------------+
```

**Use Case**

- High Performance Computing (HPC)
- Low Latency Applications
- Applications requiring fast communication between instances

---

## Partition Placement Group

Purpose: Divide instances into separate partitions. Each partition uses a separate set of underlying hardware.
```text
Partition 1       Partition 2       Partition 3

+----------+      +----------+      +----------+
| EC2 EC2  |      | EC2 EC2  |      | EC2 EC2  |
| Hardware |      | Hardware |      | Hardware |
+----------+      +----------+      +----------+
```
**Use Case**

- Hadoop
- Kafka
- Cassandra

---

## Spread Placement Group

Purpose: - Each instance is placed on separate hardware.

```text
+---------+    +---------+    +---------+
| EC2 #1  |    | EC2 #2  |    | EC2 #3  |
|Hardware1|    |Hardware2|    |Hardware3|
+---------+    +---------+    +---------+
```
**Use Case**

- Critical Applications
- Small number of important instances
- Reducing the risk of correlated hardware failures
- High availability

### Best Practices

- Cluster → Maximum Performance
- Partition → Large Distributed Systems
- Spread → Maximum Fault Isolation

### Production Example

Financial applications deploy critical servers using Spread Placement Groups to reduce the impact of hardware failures.

---

# 📊 Monitoring

### Definition

Monitoring is the process of tracking the health and performance of EC2 instances.

### Amazon CloudWatch Metrics

- CPU Utilization
- Network In
- Network Out
- Disk Read Operations
- Disk Write Operations
- Status Check Failed

### Monitoring Types

| Type | Frequency |
|--------|-----------|
| Basic Monitoring | Every 5 Minutes |
| Detailed Monitoring | Every 1 Minute |

### Best Practices

- Enable Detailed Monitoring for production workloads.
- Configure CloudWatch Alarms.
- Send notifications using Amazon SNS.
- Continuously monitor CPU, memory, and disk usage.

### Production Example

If CPU Utilization remains above 80% for five minutes, CloudWatch triggers an alarm that automatically notifies the operations team through Amazon SNS.

---
# 🚨 EC2 Production Troubleshooting Scenarios

---

## Scenario 1: Unable to SSH into EC2

### Symptoms
- SSH connection timeout
- Permission denied
- Connection refused

### Possible Causes
- Port 22 blocked in Security Group
- Wrong Key Pair
- Incorrect Public IP
- EC2 Instance stopped
- NACL blocking traffic

### Troubleshooting
- Verify EC2 is running.
- Check Security Group allows TCP 22.
- Confirm the correct Public IP.
- Verify the private key (.pem).
- Review NACL rules.
- Check the SSH service status from the console (if possible).

### Resolution
Allow SSH from your IP, use the correct key pair, and ensure the instance is reachable.

---

## Scenario 2: Website is Not Accessible

### Symptoms
- Website not opening
- Browser timeout
- 502/503 errors

### Possible Causes
- Apache/Nginx service stopped
- HTTP/HTTPS blocked
- Application crashed
- Wrong DNS
- Load Balancer issue

### Troubleshooting
- Verify EC2 is running.
- Check web server status.
- Verify Security Group (80/443).
- Review application logs.
- Check DNS configuration.

### Resolution
Restart the web service and fix the failed component.

---

## Scenario 3: High CPU Utilization

### Symptoms
- Slow application
- High response time

### Possible Causes
- High user traffic
- Infinite loop
- Java process consuming CPU
- Cron job

### Commands

```bash
top
htop
ps -ef
```

### Resolution

- Kill unnecessary process.
- Restart application.
- Scale the instance or Auto Scaling Group.

---

## Scenario 4: High Memory Usage

### Symptoms

- Server becomes slow
- Out Of Memory errors

### Commands

```bash
free -m
top
vmstat
```

### Resolution

- Restart memory consuming service.
- Increase RAM.
- Fix memory leak.

---

## Scenario 5: Disk Full

### Symptoms

- Unable to write logs
- Application crash

### Commands

```bash
df -h
du -sh /*
find / -type f -size +500M
```

### Resolution

- Delete unnecessary files.
- Rotate logs.
- Extend EBS Volume.

---

## Scenario 6: EC2 Status Check Failed

### Symptoms

AWS Console shows

Status Check Failed

### Possible Causes

- Hardware issue
- OS issue
- Kernel panic

### Resolution

- Reboot instance.
- Review system logs.
- Recover or replace instance if required.

---

## Scenario 7: Instance Not Reachable

### Possible Causes

- Wrong Route Table
- Internet Gateway missing
- NACL issue
- Security Group issue
- Public IP missing

### Resolution

Verify VPC networking configuration.

---

## Scenario 8: Public IP Changed

### Cause

Instance was stopped and started.

### Resolution

Use an Elastic IP for servers requiring a static public address.

---

## Scenario 9: Application Down After Deployment

### Possible Causes

- Application failed to start
- Configuration issue
- Missing dependency

### Resolution

- Check application logs.
- Roll back the deployment.
- Restart the service.

---

## Scenario 10: User Data Script Failed

### Symptoms

Application not installed after launch.

### Troubleshooting

```bash
cat /var/log/cloud-init-output.log
```

### Resolution

Correct the script and relaunch the instance.

---

## Scenario 11: Unable to Download Packages

### Possible Causes

- Internet Gateway missing
- NAT Gateway issue
- DNS issue

### Resolution

Verify outbound internet connectivity.

---

## Scenario 12: Instance Accidentally Terminated

### Resolution

- Launch a new instance from AMI.
- Restore EBS Snapshot.
- Recover application using backups.

---

## Scenario 13: CloudWatch Alarm Triggered

### Example

CPU > 80%

### Resolution

- Identify high CPU process.
- Restart service if required.
- Scale resources.

---

## Scenario 14: Storage Running Out Frequently

### Resolution

- Enable log rotation.
- Monitor disk usage.
- Increase EBS size.
- Archive old data.

---

## Scenario 15: Login Successful but Server Very Slow

### Possible Causes

- High CPU
- High Memory
- Disk Utilization
- Heavy Application Load

### Commands

```bash
top
free -m
iostat
vmstat
df -h
```

### Resolution

Identify the bottleneck and optimize or scale resources.

# How Many Ways to SSH into a Private EC2 Instance?

There are multiple ways to securely connect to a Private EC2 instance.

---

# 1. Bastion Host (Jump Server)

Architecture

Laptop
↓
Public EC2 (Bastion Host)
↓
Private EC2

Advantages
- Simple to implement
- Widely used

Disadvantages
- Requires Public EC2
- Maintenance required
- Additional cost

---

# 2. EC2 Instance Connect Endpoint (EICE)

Architecture

Laptop
↓
EC2 Instance Connect Endpoint
↓
Private EC2

Advantages
- No Bastion Host
- No Public IP
- AWS Managed
- Secure SSH Access

Best Use Case
- Small and Medium AWS environments

---

# 3. AWS Systems Manager Session Manager

Architecture

Laptop
↓
AWS Systems Manager
↓
SSM Agent
↓
Private EC2

Advantages
- No SSH Keys
- No Port 22 Required
- Session Logging
- AWS Managed

Best Use Case
- Production Environment (Recommended)

---

# 4. Site-to-Site VPN

Architecture

On-Premises Network
↓
VPN Tunnel
↓
AWS VPC
↓
Private EC2

Advantages
- Secure Private Network
- Enterprise Ready

Best Use Case
- Corporate Office Connectivity

---

# 5. AWS Client VPN

Architecture

Laptop
↓
AWS Client VPN
↓
AWS VPC
↓
Private EC2

Advantages
- Secure Remote Access
- Easy for Remote Employees

Best Use Case
- Work From Home Users

---

# 6. AWS Direct Connect

Architecture

On-Premises
↓
Dedicated Fiber Connection
↓
AWS VPC
↓
Private EC2

Advantages
- High Bandwidth
- Low Latency
- Highly Secure

Best Use Case
- Large Enterprises


---

## ⭐ Interview Tip

During interviews, always explain troubleshooting in this order:

1. Understand the issue.
2. Verify the EC2 instance state.
3. Check networking (VPC, Route Table, IGW, NACL, Security Group).
4. Check OS health (CPU, Memory, Disk).
5. Check application logs.
6. Implement the fix.
7. Monitor the service after recovery.
8. Perform RCA and document preventive actions.
